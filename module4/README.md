...
- ArUco-граф 5 × 5 для физического поля или 6 × 6 для Webots;
...
Для показанной в PDF физической площадки ничего задавать не нужно: используется
сетка 5 × 5, ID 0–24. В Webots перед запуском моста задайте его сетку 6 × 6:

```bash
export MVCH_FIELD_SIZE=6
```
...
Дополнительно используются `/map`, реальный
`/RMC2/camera_bottom/aruco_id` (и старый `/RMC2/aruco_id` для Webots), а также
`/RMC2/lift_status`. По документации реального RMC2 статус лифта имеет QoS
`Transient Local` и значения `lowered`, `moving`, `raised`; команды — `0.0` вниз
и `0.05` м вверх. В Webots верхнее положение — `0.1` м, поэтому это значение
используется по умолчанию. Для физического RMC2 запускайте мост так:

```bash
MVCH_LIFT_UP_HEIGHT=0.05 bun run start
```
