...
На физическом роботе `--sim` не указывать:

```bash
python3 module2/module_b.py --target 14
```

Тогда используется подтверждённый документацией профиль: поле 5×5 (ID 0–24),
ArUco `/RMC2/camera_bottom/aruco_id`, TF `/RMC2/tf` и `/RMC2/tf_static`, габарит
RMC2 до 0.870 м. С `--sim` остаётся прежний профиль Webots 6×6 (ID 0–35),
`/RMC2/aruco_id` и глобальные `/tf`/`/tf_static`.

Перед физическим заездом всё равно проверить частоту топиков, имена frame_id, шаг
сетки, тормозной путь и аппаратную аварийную остановку. Документ не сообщает шаг
между маркерами и точные имена TF-фреймов. Испытания в Webots не подтверждают
готовность к реальному полю.
Если значения отличаются, код менять не нужно:

```bash
python3 module2/module_b.py --target 14 --rows 5 --columns 5 --spacing 1.0 \
  --base-frame RMC2/base_link --odom-frame RMC2/odom
```
...
Координаты реального поля: `0=(0,0)`, `5=(-1,0)`, `1=(0,1)`; в Webots 6×6:
`0=(0,0)`, `6=(-1,0)`, `1=(0,1)`.
...
Профили сетки находятся в `REAL_FIELD` и `SIM_FIELD`, известные занятые клетки —
в `BLOCKED_MARKERS`. Все длины — метры.

## Проверки и логи

```bash
python3 module2/module_b.py --target 14 --dry-run       # реальное поле 5x5
python3 module2/module_b.py --target 35 --sim --dry-run # Webots 6x6
```
